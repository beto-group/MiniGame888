...
async function loadScript(dc, src, options = {}) {
  ...
  // Resolve custom cacheDir or default to MINIGAME 888 local path
  const resolvedCacheDir = cacheDir ? dc.resolvePath(cacheDir) : dc.resolvePath("_RESOURCES/DATACORE/_DONE/MINIGAME 888/data/cache/scripts");
    ...
    try {
      ...
      if (isUrl) {
        ...
        if (scriptContent === null && cacheDir) {
          const defaultCacheDir = dc.resolvePath("_RESOURCES/DATACORE/_DONE/MINIGAME 888/data/cache/scripts");
...
async function fetchAndCacheImage(dc, url) {
  const cacheDir = dc.resolvePath("_RESOURCES/DATACORE/_DONE/MINIGAME 888/data/cache/images");
  const adapter = dc.app.vault.adapter;
